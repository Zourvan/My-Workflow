...
echo "========================================="
echo " FULL DEV IDE SETUP (ZSH + NVIM + TOOLCHAIN)"
echo "========================================="
...
if ! command -v spf >/dev/null 2>&1; then
  SUPERFILE_VERSION=$(curl -s https://api.github.com/repos/yorukot/superfile/releases/latest | grep '"tag_name"' | cut -d '"' -f 4 | sed 's/^v//')
  if [ -z "$SUPERFILE_VERSION" ]; then
    echo "❌ Failed to detect Superfile version from GitHub API"
    exit 1
  fi
  ...
  case "$ARCH" in
    ...
    *)
      echo "❌ Unsupported architecture for Superfile: $ARCH"
      exit 1
      ;;
  esac

  SUPERFILE_TAR="superfile-linux-v${SUPERFILE_VERSION}-${ARCH}.tar.gz"
  echo "Downloading Superfile v${SUPERFILE_VERSION} (${ARCH})..."
  curl -fsSL -o "$SUPERFILE_TAR" \
    "https://github.com/yorukot/superfile/releases/download/v${SUPERFILE_VERSION}/${SUPERFILE_TAR}"

  tar -xzf "$SUPERFILE_TAR"
  sudo install "dist/${SUPERFILE_TAR%.tar.gz}/spf" /usr/local/bin/spf
  rm -rf dist "$SUPERFILE_TAR"
fi
